#!/bin/bash -l
#SBATCH --job-name=TokenHSI-train-terrain-bag
#SBATCH --output=output_slurm/train_terrain_bag_log.txt
#SBATCH --error=output_slurm/train_terrain_bag_error.txt
...
python ./tokenhsi/run.py --task HumanoidAdaptCarryGround2Terrain \
    ...
    --output_path /scratch/shdpm_root/shdpm0/wenleyan/tokenhsi/MMH-Terrain-bag/Terrain-bag-boxContinue-4/ \
    --wandb_name "Terrain-bag-boxContinue-4" \
    --notes "continue on 3 - 24-01-36-24, more lift reward" \
    --ergo_coeff 0.2 \
    --unwalkable_obstacles 0 \
    --resume 1 \
    --checkpoint /scratch/shdpm_root/shdpm0/wenleyan/tokenhsi/MMH-Terrain-bag/Terrain-bag-boxContinue-3/Humanoid_24-01-36-24/nn/Humanoid.pth \
